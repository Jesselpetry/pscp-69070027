...
```text
โจทย์อยากให้คำนวณบิลออกมาที่บวก vat + service_charge เรียบร้อยแล้ว

Input:
โปรแกรมรับ subtotal

Output:
โปรแกรมแสดงผล total ที่บวก vat กับ service_charge แล้วโดยที่มี logic ต่างๆ เช่น service_charge range คือ 50-1000

Constraints:
input: บรรทัดที่ 1 : จำนวนเงินรวมเฉพาะค่าอาหารและเครื่องดื่มที่ลูกค้าได้สั่งไป (จำนวนเต็ม)
output: บรรทัดที่ 1 : ผลรวมที่ลูกค้าต้องชำระเงินหลังจากได้รวมค่าบริการและ VAT 7% เรียบร้อยแล้ว (ให้แสดงเป็นจำนวนจริงทศนิยม 2 หลัก)
```
...
```text
Step 1: รับค่า subtotal
Step 2: ทำการหาค่า service_charge และเช็คว่าน้อยกว่า 50 หรือมากกว่า 1000 หรือเปล่าจะได้ตัดยอดให้อยู่ใน range
Step 3: ทำการหาค่า vat โดยนำ subtotal + service_charge มาคูณ 0.07
Step 4: ทำการแสดงผล total = subtotal + service_charge + vat ที่คำนวณมา
```
...
```text
เพราะเวลาหา service_charge มันต้องปีดให้อยู่ใน range 50-1000
```
...
```text
เพราะเวลาหา service_charge มันจะอยู๋ในการหาปกติ ไม่มีการปัดให้เข้า range
```
