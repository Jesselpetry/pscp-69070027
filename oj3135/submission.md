...
```text
โจทย์ให้จำลองการส่งของขวัญในวงกลมที่มีคน N คน เริ่มจากคนที่ 1 แล้วส่งต่อข้ามไปทีละ K คนเรื่อย ๆ การส่งจะหยุดลงเมื่อของขวัญวนกลับมาที่คนที่ 1 หรือส่งไปถึงมือขโมย (คนที่ T) แล้วให้หาจำนวนคนทั้งหมดที่ได้พิจารณาของขวัญ

Input:
รับจำนวนเต็มสามค่า N, K และ T ในบรรทัดเดียว คั่นด้วยช่องว่าง

Output:
แสดงจำนวนคนที่ได้พิจารณาของขวัญเป็นจำนวนเต็ม 1 ค่า

Constraints:
1 <= N <= 200,000, 1 <= K <= N-1 และ 1 <= T <= N ต้องคำนวณตำแหน่งการวนลูปแบบวงกลมให้ถูกต้อง (ใช้ modulo) และเงื่อนไขการหยุดต้องเช็คทั้งกรณีวนกลับมาคนที่ 1 หรือเจอขโมยก่อน
```
...
```text
เริ่มนับจากคนที่ 1 เป็นคนแรก จากนั้นใช้ while loop จำลองการส่งของขวัญและคำนวณตำแหน่งคนถัดไปในวงกลมด้วยสูตร (current - 1 + step) % people + 1 ในแต่ละรอบจะเพิ่มจำนวนคนที่ได้ตรวจของขวัญขึ้นทีละ 1 และจะหยุดการทำงานทันทีเมื่อของขวัญวนกลับมาถึงคนที่ 1 หรือส่งไปถึงมือขโมย แล้วจึงแสดงผลรวมจำนวนคนที่ได้พิจารณาของขวัญทั้งหมดออกมา
```
