...
```text
โจทย์ให้รับสตริงรหัสไพ่ เช่น QS หรือ 10C แล้วแปลงเป็นชื่อเต็ม เช่น queen of spades โดยแต้ม 2-10 ใช้เลขเดิม ส่วน A, J, Q, K แปลงเป็น ace, jack, queen, king และดอก D, H, S, C แปลงเป็น diamonds, hearts, spades, clubs
```
...
```text
ผมวางแผนแยกสตริงออกเป็นแต้มและดอกไพ่ โดยใช้ slicing ตัวสุดท้ายเป็นดอก และตัวที่เหลือก่อนหน้าเป็นแต้ม เพื่อรองรับแต้ม 10 จากนั้นใช้ if-elif แปลงเป็นชื่อเต็มแล้วพิมพ์ออกมา
```
...
```text
เหมือนแผนแรก ผมใช้ card[:-1] ดึงแต้ม และ card[-1] ดึงดอกไพ่ จากนั้นใช้ if-elif แปลงค่าแล้วพิมพ์ 'rank_name of suit_name' ออกมาตรงๆ
```
