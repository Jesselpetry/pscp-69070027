...
```text
โจทย์ให้นับว่ามีตัวเลขในช่วง A ถึง B กี่ตัว ที่หารด้วย d แล้วเหลือเศษ r

Input:
รับจำนวนนับ 4 ค่า คือ A (จุดเริ่มต้น), B (จุดสิ้นสุด), d (ตัวหาร) และ r (เศษ)

Output:
พิมพ์จำนวนตัวเลขทั้งหมดในช่วง [A, B] ที่หารด้วย d แล้วเหลือเศษ r

Constraints:
A, B, d, r เป็นจำนวนนับ โดยที่ A < B และ r < d
```
...
```text
Step 1: รับค่า `start`, `end`, `divisor`, `remainder` แปลงเป็นจำนวนเต็ม  
Step 2: กำหนดตัวแปร `count = 0`  
Step 3: วนลูป `number` ตั้งแต่ `start` ถึง `end` ถ้า `number % divisor == remainder` ให้เพิ่ม `count` ขึ้น 1  
Step 4: พิมพ์ค่า `count`  

ก่อนลงมือเขียนโค้ด คิดว่าจะใช้วิธีวนลูป `for` เพื่อเช็ค modulo ทีละตัวตามช่วงที่กำหนด ซึ่งเขียนง่ายและตรงไปตรงมา
```
...
```text
เหมือนกับแผนแรก โดยใช้วิธีวนลูป
```
